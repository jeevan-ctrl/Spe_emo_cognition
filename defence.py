...
if uploaded_file is not None:
    ...
    def predict_emotion(_model, mfcc):
        for models in model:
            prediction = models.predict(mfcc)
    ...
    def predict_emotion(models, mfcc):
        predictions = []
        ...
        emotions = ['angry', 'fear', 'disgust', 'calm', 'happy', 'neutral', 'sad', 'surprise']
        emotion_prediction = dict(zip(emotions, avg_prediction[0]))
    ...
    final_emotion_prob_percentage = "{:.2f}".format(emotion_prediction * 100)
    left_column.subheader("Final Emotion")
    if emotion_prediction:
        left_column.write(f"<p style='font-size:16pt'>The emotions in the sound are {', '.join(emotion_prediction)} </p>", unsafe_allow_html=True)
    else:
        left_column.write(f"<p style='font-size:16pt'>The emotion in the sound is {emotion_prediction} </p>", unsafe_allow_html=True)
